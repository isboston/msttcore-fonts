...
function list_cab_contents {
  ...
  cabextract --lowercase -l -F '*.ttf' "$cab" 2>/dev/null |
...
if ! [ "$DOWNLOAD_ONLY_FLAG" ]; then
  ...
  if command -v mkfontscale >/dev/null 2>&1 && command -v mkfontdir >/dev/null 2>&1; then
    mkfontscale "$FONTDIR" || :
    mkfontdir "$FONTDIR" || :
  else
    progress "mkfontscale/mkfontdir not found, skipping core X fonts index"
  fi
  ...
  if command -v xset >/dev/null 2>&1; then
    ...
    progress "xset not found, skipping temporary core X font path update"
  fi
